 ...
 * 重放同一批流必得同一判词（置吏不收贿）。
 *
 * 合审序（docs/03 §8 锁死）：若各流**全部**调用的 at 皆为有限数 → 全局稳定排序键
 * (at, 流序, 流内序)；否则按参序拼接（审计方须自证流序）。疾账全局：跨会话互认
 * （甲会话的疾笔与痊笔救/挂乙会话的愈言——合审即全库）；同径之愈稿同样归并
 * （甲会话写下的稿由乙会话的续写换下）。
 */

import { basename } from 'node:path'
import { parseStream, buildCalls } from './stream.js'
import { createEngine, recordCall, judge } from './jizhang.js'
