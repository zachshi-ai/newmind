...
只审「量尺本身信不信」（t>0 运行中）：知止数「验过没有」（证据有无），法仪审「尺弯没弯、罩没罩住末笔」（证据的结构条件）；定分治跨会话写域之争（横向），法仪治单会话自弯其尺（纵向）；治未病诊任务书里**有没有**验收（t=0），法仪审运行中验收器**脏没脏**（t>0）；直笔管「绿有没有被洗」（`npm test || true` 在法仪账上是绿验、在直笔账上是讳笔——绿之真伪归直笔，尺为什么绿归法仪）。不拦动作（zhizhi）、不审判断账本（jiebi）、不守本愿（zhengnian）、不勘势途（jiubian）、不管复命（youya——验尺重跑非遗忘）、不审输入权威（lunshi）、不裁写域（dingfen）、不追承诺（licheng）、不审出境（baihe）、不管行前退路（yuli）、不量总量花销（duzhi）、不审须柄授命（erbing）、不记跨会话教训（buer）。词表互斥见 docs/04 A11（grep 无输出）。
