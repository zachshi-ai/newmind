#!/usr/bin/env node
/**
 * 察传 CLI —— 零依赖无见立言审计器（多流离线合审，可验尸任何历史会话）。
 *
 ...
 *   chachu list [--file <证册>]
 *   chachu block [--file <证册>]
 *   chachu gate --value <n> [--gate n]
 *   chachu --help | --version
 *
 * 退出码：0 通过；1 门禁失败；2 用法/输入错误。
 * 无册照判：audit 不带 --file 或默认册缺失时 excuse/grounds 为空——凡言必据
 * （断言之责不挑文件；得言而不察物，与穿井得一人只差一个读者的距离）。
 */

import { readFileSync, writeFileSync, existsSync } from 'node:fs'
import { resolve } from 'node:path'
import { auditStreams } from '../core/audit.js'
import { GATE_DEFAULT, bandOf } from '../core/shenyan.js'
import { emptyBook, parseBook, registerEntry, revokeEntry, serializeBook, bookCount } from '../core/zhengce.js'
import { renderZhengpai } from '../core/zhengpai.js'

const VERSION = '0.1.0'

const USAGE = `察传 · chachu —— DeepSeek Harness 无见立言治理的离线 CLI（得言不可以不察）

用法:
  chachu audit <s1.jsonl> [s2.jsonl …] [选项]     审言（得言形×指物×见据 → 幻值 + 分带 + 门禁）
  chachu register --path <glob> [选项]            立免审（本任务授权之稿，重复去重）
  chachu revoke --path <glob> [选项]
  chachu list [选项]                              阅册（证册 JSON）
  chachu block [选项]                             证牌块（证册与词法公示，逐字节确定）
  chachu gate --value <n> [选项]                  门禁裁决
  chachu --help | --version

选项:
  --file <path>      证册文件（默认 ./.chachu.json；缺失时 audit 无册全账）
  --path <glob>      免审径（register/revoke 可用；* 跨目录）
  --gate <n>         幻值阈门（默认 ${GATE_DEFAULT}：≥30 为「诞」，退出码 1）
  --json             紧凑 JSON 输出

退出码: 0 通过；1 门禁失败；2 用法/输入错误。`

function fail(message, code = 2) {
  console.error(`chachu: ${message}`)
  process.exit(code)
}
...
function registryPath(args) {
  ...
  if (idx === -1) return resolve(process.cwd(), '.chachu.json')
  const v = args[idx + 1]
...
function cmdAudit(args) {
  ...
  if (compact) {
    ...
    console.log(`案账：幻言 ${res.counts.hy} · 疑言 ${res.counts.yy} · 迟证 ${res.counts.cz} · 虚指 ${res.counts.xz}`)
    console.log(`幻值 ${res.score.total}（huan ${res.score.huan} + yi ${res.score.yi}）· 带「${res.band}」· 门 ${res.gate} → ${res.verdict === 'pass' ? '过' : '红'}`)
...
function cmdBlock(args) {
  const { book } = loadBook(args)
  console.log(renderZhengpai(book, { counts: { hy: 0, yy: 0, cz: 0, xz: 0 }, issues: ['言皆有据 ×0 稿 0 行 —— 言必有据，指物在先'] }))
}
...
function cmdGate(args) {
  ...
  console.log(`幻值 ${value} · 带「${bandOf(value)}」· 门 ${gate} → ${pass ? '过' : '红'}`)
