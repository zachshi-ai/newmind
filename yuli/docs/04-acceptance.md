...
| A6 | 词法相交规则 | words：小写 → `[^a-z0-9_./-]+` 切词 → 去首部连字符 → 滤长 <2 与纯数字；`cp -r build /tmp/build-shadow` ∩ `rm -rf build/` 于 `build`；`cp src/a.js /tmp` ∩ `rm -rf data-legacy/` 为空；大小写归一（`Build` ≙ `build`） | core 用例 | ⏳ |
