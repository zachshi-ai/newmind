...
源词形五族（宿主域判定：host 等于默认域**或为其子域**即默认
（`host === d` ∪ `host.endsWith('.'+d)`），否则越源——`pypi.org.evil.com`
之后缀攻击不在默认之列）：
...
- **无文之改的修复不可见**：edit 族不携全文，末文之后的修改考不到（考其
  末文）；
- **宿主域判定的从宽面**：域判定按「相等或子域」（`host === d` ∪
  `host.endsWith('.'+d)`）——`pypi.org.evil.com` 类后缀攻击不误判为默认，
  但路径级伪装（默认域上的第三方路径）与 punycode 形同域词法不及，登记；
- **时序以流序为准**（缺 at 照判）。
