/**
 * 照册 —— 免审之册的持久化（docs/03 §10 锁死）。
 *
 * 册形：{ "version": 1, "allow": ["reports/internal/*"], "shapes": ["changelog"], "noDefaults": false }
 * 声明权全在任务方：allow 免审是「此径内文书所弃之责皆有名分」的明言；无册照判
 * （凡弃必凭——弃责之责不挑文件，豁免授权只能来自册）。register/revoke 归 CLI；
 * 插件只吃注入的 book 对象。v1 allow 只豁免；shapes 增责面形；noDefaults 关默认表；
 * 弃责形/否定卫/镜形内置固定，不扩形（弱切割词宁纵不收）。
 */
...
export function parseBook(text) {
  ...
  try {
    ...
    throw new Error('照册不是合法 JSON')
  }
  if (!v || typeof v !== 'object' || Array.isArray(v)) throw new Error('照册须为对象')
...
export function revokeEntry(book, glob) {
  ...
  if (idx === -1) throw new Error(`照册无此免审径: ${g}`)
