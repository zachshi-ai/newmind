/**
 * 度支流解析 —— 兼容 zhizhi / jiebi / zhengnian / weibing / jiubian / youya / lunshi /
 * dingfen / baihe / zhibi 的会话流格式（全仓同构，跨项目互审的前提）。
 *
 ...
 * 非工具事件（turn_start / turn_end 等）跳过。
 * `at` 原样保留——度支的时程与时程过线判定都吃它；无 at 之流时长维度诚实退化。
 */
...
/** 归并出调用序列（流序）。返回 { calls }，call: { ref, name, args, isError, at }。 */
