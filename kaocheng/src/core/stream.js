/**
 * 考诚流解析 —— 兼容 zhizhi / jiebi / zhengnian / jiubian / mingshi / xiangxiao / zhizu / shefa / yuanyu 的会话流格式。
 *
 ...
 * 本层判定只用流内序列（seq），不依赖时间戳；写族交付内容取自 args.content（物之末据的词面证据随流携带），
 * 结果侧 content 不参与判定（观察不是工，读取不豁免契约）。
 */
