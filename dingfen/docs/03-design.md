...
1. **争写**（流间事实，另算，见 §6）；
2. **侵入**：分册存在，且存在**他方**（id≠S）之分在 `t` 开放且其某 glob 命中 `k`（仅 `p:` 键判）→ 侵入。按（会话, 对象）去重——两个会话各闯同一路径是两宗，同一会话闯同一路径两次是一宗。
3. **越分**：未判侵入，且 S 自己的分在 `t` 开放、但其全部 glob 都不命中 `k`（仅 `p:` 键判）→ 越分（同样按（会话, 对象）去重）。
4. **未领分**：未判侵入，且 S 在 `t` 无开放之分 → 提示级，**不计分**（声明权在账方；按写事件计件，不去重）。
5. 皆非 → 守分之写，无话。

判定序锁死：**侵入 > 越分 > 未领分**（同一写不得双计——闯了别人的分，罪在侵入，不在漂移）。无分册时 2/3/4 全部不判（纯流间审计）。无时之写（`at` 缺失）同样不判 2/3——开放时段不可考，宁可放过，与 §1 的无时之则一致。
...
|---|---|---|---|
| 流间 | 争写 | +30 / 处 | 60 |
| 权界 | 侵入 | +30 / 径（按会话×对象去重） | 60 |
| 纪律 | 越分 | +6 / 径（按会话×对象去重） | 30 |
...
- `name='dingfen'`，`inject=['tools']`，config：`{ fences?, sessionId?, registry?, gate?, now? }`（`now` 是时钟注入口，缺省 `Date.now`；测试注入确定性时钟，杜绝同毫秒排序侥幸）。
...
### stray.jsonl + stray-registry（权界夹具）
