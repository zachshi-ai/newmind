/**
 * 瑕册 —— 免审之册的持久化（docs/03 §9 锁死）。
 *
 * 册形：{ "version": 1, "excuse": ["docs/reports/*"] }
 * 声明权全在任务方：免审是「本任务授权之径」的明言；无册照判（凡卷皆审）；
 * register/revoke 归 CLI，插件只吃注入的 book 对象。v1 只豁免不扩形（词表内置固定）。
 */
...
export function parseBook(text) {
  ...
  try {
    ...
    throw new Error('瑕册不是合法 JSON')
  }
  if (!v || typeof v !== 'object' || Array.isArray(v)) throw new Error('瑕册须为对象')
...
export function revokeEntry(book, glob) {
  ...
  if (idx === -1) throw new Error(`瑕册无此免审径: ${g}`)
