...
| A3 | CLI 语义 | `template` 输出合法契约骨架；`contract`：合法 → 0，非法/坏 JSON → 2；`audit`：尘值超阈或终验未对账（--acceptance）→ 1，坏输入（不可读/坏 JSON/非法契约）→ 2，干净 → 0；`--gate` / `--max-stale` / `--acceptance` / `--cwd` / `--stream` 生效；`reanchor` 输出拂拭块文本（`--json` 包装） | CLI 测试 | ✅ 12 用例全绿 |
...
ℹ pass 74
ℹ fail 0
（core 51 + cli 12 + 集成 11，分文件运行各自全绿）
```
