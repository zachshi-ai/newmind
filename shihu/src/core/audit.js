/**
 * 多流合审 —— 每个流文件是一个会话，合入同一引擎出统一判词（声明按其所在
 * 会话的作工面对账——自态自证，他会话的作工救不了本会话的虚报）。
 * 条目形如 { name, text }（文件由 CLI 读好，core 不触文件系统）；撞名报错（exit 2）。
 * 重放同一批流必得同判词（置吏不收贿）。
 *
 * 合审序（docs/03 §2 锁死）：若各流**全部**调用的 at 皆为有限数 → 全局稳定
 * 排序键 (at, 流序, 流内序)；否则按参序拼接（审计方须自证流序）。
 */

import { basename } from 'node:path'
import { parseStream, buildCalls } from './stream.js'
import { createEngine, recordCall, judge } from './huyin.js'
