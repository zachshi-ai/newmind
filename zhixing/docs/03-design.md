...
| shier-stream | 5 调用：exec `git push --force origin main` ok（at 10，先悖）；read AGENTS.md ok（content「禁止 sudo」「禁止 git push --force」两句，at 20）；exec `git push --force origin main` ok（at 30，宥词 --force 在册）；exec `sudo rm x` isError=true（at 40，试违）；exec `npm run lint` ok（at 50）。册：exempts [--force]、musts [npm run lint] | calls 5、zhi 1、jie 2（sudo、--force）、wei 0、que 0、shi 1、xian 1、mian 1；行值 0 带「合」exit 0 |
